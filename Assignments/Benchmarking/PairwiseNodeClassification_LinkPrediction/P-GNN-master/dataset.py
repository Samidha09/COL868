...
def get_tg_dataset(args, dataset_name, use_cache=True, remove_feature=False):
    ...
    if dataset_name in ['Cora', 'CiteSeer', 'PubMed']:
        dataset = tg.datasets.Planetoid(
            root='datasets/' + dataset_name, name=dataset_name)
    else:
        try:
    ...
    f1_name = 'datasets/cache/' + dataset_name + \
        str(args.approximate) + '_dists.dat'
    f2_name = 'datasets/cache/' + dataset_name + \
        str(args.approximate) + '_dists_removed.dat'
    f3_name = 'datasets/cache/' + dataset_name + \
        str(args.approximate) + '_links_train.dat'
    f4_name = 'datasets/cache/' + dataset_name + \
        str(args.approximate) + '_links_val.dat'
    f5_name = 'datasets/cache/' + dataset_name + \
        str(args.approximate) + '_links_test.dat'
    ...
    if use_cache and ((os.path.isfile(f2_name) and args.task == 'link') or (os.path.isfile(f1_name) and args.task != 'link')):
        ...
        for i, data in enumerate(dataset):
            ...
            if args.task == 'link':
                ...
                ), data.num_nodes, approximate=args.approximate)
...
def load_graphs(dataset_str):
    ...
    if dataset_str == 'grid':
        ...
        graphs_all, features_all, labels_all = Graph_load_batch(
            name='PROTEINS_full')
        features_all = (features_all-np.mean(features_all, axis=-1,
                                             keepdims=True))/np.std(features_all, axis=-1, keepdims=True)
        graphs = []
        features = []
        edge_labels = []
        ...
        comps = [comp for comp in nx.connected_components(
            graph) if len(comp) > 10]
        ...
        edge_labels_internal = {
            int(i): l for i, l in edge_labels_internal.items()}
